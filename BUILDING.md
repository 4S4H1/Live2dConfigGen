...
推荐在编辑器的“工具 → 局域网更新主机…”中唤醒独立托盘 Host，再选择
“导入签名更新包…”。
Host 的 HTTP 端口可配置；自动发现使用独立的固定 UDP `48765`。客户端会向
各活动 IPv4 网卡的广播地址、受限广播地址和 `127.0.0.1` 发送带随机 nonce
的短请求，响应只包含相关 nonce 与 HTTP 端口。发现结果不建立信任，后续仍
必须通过内嵌公钥完成清单签名与安装器哈希验证。
命令行等价操作：
...
v1 清单使用固定 `key_id: release-1`，客户端和独立更新主机只信任安装时内嵌的单一
公钥，不支持从 HTTP 自动获取或替换信任根。不得只换私钥后直接发布，否则
所有已安装客户端都会拒绝更新。需要轮换时，先用旧私钥发布并安装一个同时
内嵌新公钥的桥接版本，再切换后续发布密钥。更新主机与客户端随同一个
安装器更新公钥。
...
- 全新安装、覆盖升级、卸载后用户 JSON 与编辑器设置仍存在；Host 端口、
  服务状态和登录恢复偏好随 Host 组件一并移除，发布缓存仍须保留。
- 从有注册项、自定义目录或仅剩默认目录的旧版独立 Host 升级后，迁移为
  `%LOCALAPPDATA%\Programs\L2DUpdateHost` 中的伴随组件，移除旧卸载入口和
  精确快捷方式，并保留发布缓存。
...
- 无已保存地址时可通过 UDP 自动发现；Host 与客户端在同一台电脑时可通过
  显式 loopback 探测完成自更新。
- 验证 1.0.0 → 1.1.0 的首次引导需直接运行 Setup，或在 1.0.0 中配置
  `http://127.0.0.1:<HTTP 端口>`；UDP 自动发现从 1.1.0 客户端开始生效。
- UDP `48765` 被占用时 HTTP Host 继续工作，并明确提供复制地址的手动回退。
- 篡改清单、签名、安装器或版本降级均被拒绝。
- 更新主机只响应白名单文件的 `GET`/`HEAD`，路径穿越和写请求被拒绝。
- 防火墙规则只包含独立 Host EXE、HTTP TCP 端口、发现 UDP `48765`、
  Private/Domain 和 LocalSubnet。
