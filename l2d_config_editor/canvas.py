...
from .widgets import (
    APPEARANCE_DISABLED_NODE_TYPES,
    APPEARANCE_BUTTON_HIDDEN_NODE_TYPES,
    CommitComboBox,
    CommitLineEdit,
    NodeAppearanceDialog,
    NodeFormWidget,
    NumericLineEdit,
    _move_dialog_near_anchor,
)

performance_recorder = get_performance_recorder()


class InlineTitleLineEdit(QLineEdit):
...
class InlineTitlePlainTextEdit(QPlainTextEdit):
    submitted = Signal(str)
    cancelled = Signal()
...
class GridScene(QGraphicsScene):
    ...
    def __init__(self, parent=None) -> None:
        ...
        self.theme_palette = palette_for_theme(ThemeMode.DARK)
        self.setSceneRect(-1_000_000, -1_000_000, 2_000_000, 2_000_000)
        self.minor_grid = 20
        self.major_grid = 100
        self.top_right_hint = ""
        self.bottom_right_hint = "按住中键平移 / 滚轮缩放 / Ctrl+V 粘贴截图 / Delete 删除"

    def set_ui_theme(self, mode: ThemeMode | str) -> None:
        self.theme_palette = palette_for_theme(mode)
    ...
    def drawBackground(self, painter: QPainter, rect: QRectF) -> None:
        with performance_recorder.measure(
            ...
            painter.fillRect(rect, QColor(self.theme_palette.canvas_background))
            ...
            if minor_lines:
                painter.setPen(QPen(QColor(self.theme_palette.grid_minor), 1))
                painter.drawLines(minor_lines)
            painter.setPen(QPen(QColor(self.theme_palette.grid_major), 1))
            if major_lines:
    ...
    def drawForeground(self, painter: QPainter, rect: QRectF) -> None:
        ...
        hint_color = QColor(self.theme_palette.canvas_hint)
        hint_color.setAlpha(86)
...
def build_connection_curve(start: QPointF, end: QPointF) -> QPainterPath:
    """Build the shared cubic path used by stored and preview connections."""
    path = QPainterPath(start)
    dx = end.x() - start.x()
    control_offset = max(80.0, min(260.0, abs(dx) * 0.48))
    if dx < 0:
        control_offset = max(control_offset, 140.0)
    path.cubicTo(
...
class ConnectionCurveLookup:
    """Small arc-length lookup used to place effects uniformly on a curve."""

    SEGMENT_COUNT = 16

    def __init__(self, path: QPainterPath) -> None:
        self._points = tuple(path.pointAtPercent(index / self.SEGMENT_COUNT) for index in range(self.SEGMENT_COUNT + 1))
        distances = [0.0]
        for start, end in zip(self._points, self._points[1:]):
            distances.append(distances[-1] + math.hypot(end.x() - start.x(), end.y() - start.y()))
        self._distances = tuple(distances)
        self.total_length = distances[-1]

    def sample(self, position: float) -> tuple[QPointF, QPointF]:
        position = max(0.0, min(1.0, float(position)))
        if self.total_length <= 0.001:
            return QPointF(self._points[0]), QPointF(1.0, 0.0)
        target_distance = self.total_length * position
        index = max(1, min(len(self._distances) - 1, bisect_left(self._distances, target_distance)))
        start = self._points[index - 1]
        end = self._points[index]
        segment_length = self._distances[index] - self._distances[index - 1]
        if segment_length <= 0.001:
            return QPointF(start), QPointF(1.0, 0.0)
        ratio = (target_distance - self._distances[index - 1]) / segment_length
        point = QPointF(
            start.x() + (end.x() - start.x()) * ratio,
            start.y() + (end.y() - start.y()) * ratio,
        )
        tangent = QPointF((end.x() - start.x()) / segment_length, (end.y() - start.y()) / segment_length)
...
class ConnectionItem(QGraphicsPathItem):
    def __init__(self, view: "NodeCanvasView", from_uuid: str, to_uuid: str) -> None:
        ...
        self._curve_lookup: ConnectionCurveLookup | None = None
        self.setZValue(-8)
    ...
    def boundingRect(self) -> QRectF:
        return super().boundingRect().adjusted(-16.0, -16.0, 16.0, 16.0)
    ...
    def update_path(self) -> None:
        ...
        if start is None or end is None:
            return
        path = build_connection_curve(start, end)
        self.setPath(path)
        self._curve_lookup = ConnectionCurveLookup(path)

    def _draw_direction_arrow(self, painter: QPainter, color: QColor) -> None:
        self._draw_direction_arrow_at(painter, 0.58, color, 9.0 if not self.view.should_use_motion_preview() else 7.0)
    ...
    def _draw_direction_arrow_at(
        ...
        if self._curve_lookup is None:
            return
        length = self._curve_lookup.total_length
        if length < 24.0:
            return
        position = max(0.08, min(0.92, position))
        tip, tangent = self._curve_lookup.sample(position)
        ux = tangent.x()
        uy = tangent.y()
    ...
    def _draw_flow_arrows(self, painter: QPainter, color: QColor) -> None:
        if self._curve_lookup is None:
            return
        length = self._curve_lookup.total_length
        if length < 36.0:
        ...
        while position < 0.94:
            self._draw_direction_arrow_at(painter, position, color, 7.2)
            position += spacing

    def _draw_flow_pulses(self, painter: QPainter, color: QColor) -> None:
        if self._curve_lookup is None:
            return
        length = self._curve_lookup.total_length
        if length < 42.0:
            return
        phase = self.view.connection_flow_phase()
        spacing = max(0.16, min(0.30, 68.0 / length))
        position = 0.08 + phase * spacing
        pulse_color = QColor(color)
        pulse_color.setAlpha(218)
        tail_color = QColor(color)
        tail_color.setAlpha(92)
        while position < 0.96:
            center, tangent = self._curve_lookup.sample(position)
            tail = QPointF(center.x() - tangent.x() * 22.0, center.y() - tangent.y() * 22.0)
            painter.setPen(QPen(tail_color, 3.0, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap))
            painter.drawLine(tail, center)
            painter.setPen(Qt.PenStyle.NoPen)
            painter.setBrush(pulse_color)
            painter.drawEllipse(QRectF(center.x() - 3.2, center.y() - 3.2, 6.4, 6.4))
            position += spacing

    def _draw_connection_glow(self, painter: QPainter, color: QColor) -> None:
        glow = QColor(color)
        glow.setAlpha(42)
        painter.setPen(QPen(glow, 9.0, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap))
        painter.drawPath(self.path())
        core = QColor(color)
        core.setAlpha(88)
        painter.setPen(QPen(core, 4.4, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap))
        painter.drawPath(self.path())

    def paint(self, painter: QPainter, option, widget=None) -> None:
        motion_preview = self.view.should_use_motion_preview()
        fast_render = self.view.should_use_fast_rendering()
        ...
        is_active = is_related or self.isSelected()
        detailed_effects = self.view.should_use_detailed_connection_effects()
        palette = self.view.theme_palette
        color = (
            QColor(palette.connection_related)
            if is_related
            else QColor(palette.connection_selected if self.isSelected() else palette.connection_normal)
        )
        ...
        if is_active and not fast_render and detailed_effects:
            self._draw_connection_glow(painter, color)
        painter.drawPath(self.path())
        if self._curve_lookup is not None:
            if is_active and not fast_render and detailed_effects:
                self._draw_flow_pulses(painter, color)
                self._draw_flow_arrows(painter, color)
            elif is_active and not fast_render:
                position = 0.12 + self.view.connection_flow_phase() * 0.76
                self._draw_direction_arrow_at(painter, position, color, 7.2)
            else:
                self._draw_direction_arrow(painter, color)


class TemporaryConnectionItem(QGraphicsPathItem):
    def __init__(self, view: "NodeCanvasView") -> None:
        super().__init__()
        self.view = view
    ...
    def boundingRect(self) -> QRectF:
        return super().boundingRect().adjusted(-18.0, -18.0, 18.0, 18.0)

    def set_target_active(self, active: bool) -> None:
        if self._target_active == active:
            return
    ...
    def paint(self, painter: QPainter, option, widget=None) -> None:
        ...
        painter.setRenderHint(QPainter.RenderHint.Antialiasing, not fast_render)
        painter.setBrush(Qt.BrushStyle.NoBrush)
        color = QColor(self.view.theme_palette.connection_preview)
        if self._target_active:
            color = color.lighter(118)
        if not fast_render:
            glow = QColor(color)
            glow.setAlpha(68 if self._target_active else 40)
            painter.setPen(QPen(glow, 10.0 if self._target_active else 7.0, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap))
            painter.drawPath(self.path())
        stroke = QColor(color)
        stroke.setAlpha(238)
        painter.setPen(QPen(stroke, 2.4 if self._target_active else 1.8, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap))
        painter.drawPath(self.path())
        if fast_render:
            return
        if self._curve_lookup is None:
            return
        length = max(1.0, self._curve_lookup.total_length)
        phase = self.view.connection_flow_phase()
        spacing = max(0.18, min(0.34, 58.0 / length))
        position = 0.1 + phase * spacing
        pulse = QColor("#ffffff" if self._target_active else self.view.theme_palette.connection_preview)
        pulse.setAlpha(230)
        tail = QColor(color)
        tail.setAlpha(110)
        while position < 0.96:
            center, tangent = self._curve_lookup.sample(position)
            back = QPointF(center.x() - tangent.x() * 20.0, center.y() - tangent.y() * 20.0)
            painter.setPen(QPen(tail, 3.0, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap))
            painter.drawLine(back, center)
            painter.setPen(Qt.PenStyle.NoPen)
            painter.setBrush(pulse)
            radius = 4.0 if self._target_active else 3.1
...
class CanvasStrokeItem(QGraphicsPathItem):
    ...
    def __init__(self, view: "NodeCanvasView", record) -> None:
        ...
        self.setPath(self.path_for_points(getattr(record, "points", ())))

    @staticmethod
    def point_from_record(value: Any) -> QPointF:
    ...
    @classmethod
    def path_for_points(cls, points) -> QPainterPath:
        resolved = [cls.point_from_record(point) for point in points]
        path = QPainterPath()
        if not resolved:
            return path
        path.moveTo(resolved[0])
        if len(resolved) == 1:
            path.lineTo(resolved[0] + QPointF(0.01, 0.01))
            return path
    ...
    def paint(self, painter: QPainter, option, widget=None) -> None:
        del option, widget
        color = QColor(str(getattr(self.record, "color", "#2F80ED")))
        if not color.isValid():
            color = QColor("#2F80ED")
        width = max(1.0, float(getattr(self.record, "width", 4.0)))
        halo = QColor("#ffffff" if self.view.theme_mode is ThemeMode.DARK else "#172033")
        halo.setAlpha(105)
        painter.setRenderHint(QPainter.RenderHint.Antialiasing, True)
        painter.setBrush(Qt.BrushStyle.NoBrush)
        painter.setPen(QPen(halo, width + 2.4, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap, Qt.PenJoinStyle.RoundJoin))
        painter.drawPath(self.path())
...
class CanvasImageItem(QGraphicsObject):
    ...
    def __init__(self, controller, view: "NodeCanvasView", record) -> None:
        ...
        self.view = view
        self.record = record
        self.image = decode_reference_image(record.data_base64, record.mime_type)
        self.pixmap = QPixmap.fromImage(self.image)
        self._rect = QRectF(
            0.0,
            0.0,
            max(1.0, float(record.ui_size.get("width", self.image.width() or 1))),
    ...
    def mouseReleaseEvent(self, event: QMouseEvent) -> None:
        ...
        new_position = self.view.snap_position(self.pos())
...
class NodeItem(QGraphicsObject):
    ...
    TITLE_BASE_POINT_SIZE = 16.0
    TITLE_MIN_POINT_SIZE = 2.0
    TITLE_MAX_POINT_SIZE = 100.0
    COMMENT_TITLE_SCALE = 0.7
    ...
    CARD_NOTE_BASE_POINT_SIZE = 15.0
    CARD_NOTE_MIN_POINT_SIZE = 8.0
    CARD_NOTE_MAX_POINT_SIZE = 88.0
    CARD_TITLE_BASE_POINT_SIZE = 14.0
    CARD_TITLE_MIN_POINT_SIZE = 9.0
    CARD_TITLE_MAX_POINT_SIZE = 96.0
    CARD_ACTION_BASE_POINT_SIZE = 13.0
    CARD_ACTION_MIN_POINT_SIZE = 8.0
    CARD_ACTION_MAX_POINT_SIZE = 88.0
    CARD_TARGET_BASE_POINT_SIZE = 18.0
    CARD_TARGET_MIN_POINT_SIZE = 9.0
    CARD_TARGET_MAX_POINT_SIZE = 100.0
    ...
    INITIAL_FRAME_BOTTOM_MARGIN = 18.0
    INITIAL_TITLE_BASE_POINT_SIZE = 16.8
    INITIAL_TITLE_MIN_POINT_SIZE = 13.8
    ...
    def __init__(self, schema: EditorSchema, controller, node, inline_width: int = 340) -> None:
        ...
        self._comment_editor_proxy: QGraphicsProxyWidget | None = None
        self._finishing_comment_edit = False
    ...
    def _initial_frame_rect(self) -> QRectF:
        width = self._card_layout.get("frame", QRectF()).width() if "frame" in self._card_layout else self.INITIAL_NODE_WIDTH
        height = self._card_layout.get("frame", QRectF()).height() if "frame" in self._card_layout else self.INITIAL_NODE_HEIGHT
        return QRectF(self.INITIAL_FRAME_SIDE_MARGIN, self._initial_frame_top(), width, height)

    def _initial_frame_top(self) -> float:
        note_font = self._compact_note_font_for_text("empty", self.INITIAL_NODE_WIDTH)
        return self._compact_note_height_for_font(note_font) + 16.0
    ...
    def _pin_anchor_rect(self) -> QRectF:
        ...
        if self.node.type == "Initial":
            return self._card_layout.get("frame", self._initial_frame_rect())
        return self._rect
    ...
    def input_pin_scene_pos(self) -> QPointF | None:
        if not self._supports_input_connection():
            return None
        return self.mapToScene(self._pin_center("input"))

    def output_pin_scene_pos(self) -> QPointF | None:
        if not self._supports_output_connection():
            return None
        return self.mapToScene(self._pin_center("output"))

    def input_pin_rect(self) -> QRectF:
        return self._pin_rect("input", self._pin_radius) if self._supports_input_connection() else QRectF()

    def output_pin_rect(self) -> QRectF:
        return self._pin_rect("output", self._pin_radius) if self._supports_output_connection() else QRectF()
    ...
    def lock_rect(self) -> QRectF:
        if self.node.type == "Initial":
            frame_rect = self._card_layout.get("frame", self._initial_frame_rect())
            return QRectF(frame_rect.right() - 34.0, frame_rect.top() + 10.0, 18.0, 18.0)
    ...
    def _supports_input_connection(self) -> bool:
        ...
        return self._supports_connections() and bool(definition and definition.category != "root")

    def _supports_output_connection(self) -> bool:
        return self._supports_connections()
    ...
    def pin_hit(self, scene_pos: QPointF) -> str | None:
        ...
        if self._supports_output_connection() and self._pin_rect(
            "output", self._pin_radius + self.PIN_HIT_PADDING
        ).contains(local):
            return "output"
        if self._supports_input_connection() and self._pin_rect(
            "input", self._pin_radius + self.PIN_HIT_PADDING
        ).contains(local):
            return "input"
        return None
    ...
    def update_node(self, node) -> None:
        ...
        form_mode = "advanced" if self._display_mode == "detail" else "simple"
        build_inline_form = node.type != "Comment" and (
            not self._uses_compact_card() or len(self.controller.document.nodes) <= self.LAZY_FORM_NODE_COUNT_THRESHOLD
        )
        ...
        if self._uses_compact_card():
            ...
            base_content_width = int(self.INITIAL_NODE_WIDTH - self._margin * 2)
        ...
        width = base_width
        if node.type == "Initial":
            content_origin_x = self.INITIAL_FRAME_SIDE_MARGIN
            content_origin_y = self._initial_frame_top()
            frame_width = base_width
            width = frame_width + self.INITIAL_FRAME_SIDE_MARGIN * 2.0
        if self._uses_compact_card():
            ...
            width = max(self.RESIZE_MIN_WIDTH, persisted_width or 360.0)
            height = max(
                140.0,
                float(node.ui_size.get("height", 180.0)) if node.ui_size else 180.0,
            )
            self._header_height = 0.0
            self._content_top_gap = 0.0
            self._title_rect = QRectF(20.0, 18.0, max(80.0, width - 58.0), max(60.0, height - 42.0))
            ...
            self._sync_comment_editor_geometry()
        ...
        if not self._uses_compact_card() and not self._is_draw_frame() and node.type != "Comment":
            ...
            if node.type == "Initial":
                height = self.INITIAL_NODE_HEIGHT
            if definition.resizable and node.ui_size and not compact_mode:
                height = max(height, float(node.ui_size.get("height", height)))
            if node.type in {"Comment", "Initial"} and not compact_mode:
                available_content_height = int(max(1.0, height - self._margin * 2 - self._header_height - content_top_gap))
                if node.type == "Comment":
                    available_content_height = int(max(content_height, available_content_height))
            ...
            frame_height = max(min_height, height)
            if node.type == "Initial":
                self._card_layout = {"frame": QRectF(content_origin_x, content_origin_y, frame_width, frame_height)}
                self._rect = QRectF(
                    0.0,
                    0.0,
                    width,
                    content_origin_y + frame_height + self.INITIAL_FRAME_BOTTOM_MARGIN,
                )
            else:
                self._card_layout = {}
                self._rect = QRectF(0.0, 0.0, width, frame_height)
            self._sync_form_proxy_geometry(
                final_content_width,
                content_height,
                content_origin_y + self._header_height + content_top_gap,
                left=content_origin_x + self._margin,
            )
            if node.type == "Idle0":
                self.form.hide()
                self.proxy.hide()
    ...
    def _sync_form_proxy_geometry(
        ...
        *,
        ...
        proxy_left = float(self._margin) if left is None else float(left)
        self.proxy.setGeometry(QRectF(proxy_left, float(top), float(content_width), float(content_height)))
    ...
    def _compact_field_visible(self, field_key: str) -> bool:
        view = self._canvas_view()
        definition = self.schema.nodes.get(self.node.type)
        if definition is not None and field_key not in {field.key for field in definition.fields}:
            return False
        return not view or not view.concise_enabled or field_key in view.concise_fields

    def _compact_note_layout_font(self) -> QFont:
        font = QFont(self.form.font())
        font.setBold(True)
        # Reserve enough stable geometry for the minimum on-screen font at the
        # smallest supported zoom. Zoom-only paint compensation never resizes it.
        font.setPointSizeF(
            self.CARD_NOTE_MIN_POINT_SIZE / self.CARD_TEXT_SCALE_FLOOR
        )
        return font

    def _screen_scaled_point_size(self, screen_size: float, *, minimum_screen: float, maximum_screen: float) -> float:
        target = max(minimum_screen, min(maximum_screen, float(screen_size)))
        return target / max(self.CARD_TEXT_SCALE_FLOOR, self._view_scale())

    def _compact_card_font(self, point_size: float, *, minimum_screen: float = 8.0, maximum_screen: float = 18.0) -> QFont:
        font = QFont(self.form.font())
        font.setBold(True)
        font.setPointSizeF(
    ...
    def _compact_note_layout_font_for_text(self, text: str, max_width: float) -> QFont:
        font = self._compact_note_layout_font()
        resolved_text = str(text or "").strip() or "empty"
        available_width = max(1.0, max_width - 20.0)
        text_width = QFontMetricsF(font).horizontalAdvance(resolved_text)
        if text_width <= available_width:
            return font
        current_size = font.pointSizeF()
        font.setPointSizeF(max(self.CARD_NOTE_MIN_POINT_SIZE, current_size * available_width / max(1.0, text_width)))
        return font

    def _compact_note_height_for_font(self, font: QFont) -> float:
        minimum_screen_font = self._compact_note_layout_font()
        return max(
            48.0,
            float(QFontMetricsF(font).height()) + 18.0,
            float(QFontMetricsF(minimum_screen_font).height()) + 18.0,
        )
    ...
    def _recompute_compact_card_layout(self, width: float, *, note_text: str | None = None) -> float:
        ...
        if view is not None and view.concise_enabled:
            ...
            note_font = self._compact_note_layout_font_for_text(resolved_note_text, inner_width)
            note_height = self._compact_note_height_for_font(note_font) if self._compact_field_visible("tips") else 0.0
            note_rect = QRectF(outer_margin, 8.0, inner_width, note_height) if note_height else QRectF()
            frame_top = (note_rect.bottom() + 10.0) if note_height else 8.0
            identity_height = 42.0
            ...
            for layout_key, field_key, row_height in row_specs:
                visible = self._compact_field_visible(field_key)
                if field_key == "action_trigger_active" and self.node.type == "TouchDrag":
                    visible = False
                if not visible:
                    rows[layout_key] = QRectF()
                    continue
                rows[layout_key] = QRectF(outer_margin + 18.0, cursor_y, available_width, row_height)
                cursor_y += row_height + 10.0
            frame_height = max(76.0, cursor_y - frame_top + 8.0)
            frame_rect = QRectF(outer_margin, frame_top, inner_width, frame_height)
            self._card_layout = {
                "note": note_rect,
                "identity": QRectF(frame_rect.left() + 12.0, frame_rect.top() + 7.0, frame_rect.width() - 24.0, identity_height - 8.0),
                "frame": frame_rect,
                **rows,
            }
            self._header_height = 0.0
        ...
        note_font = self._compact_note_layout_font_for_text(resolved_note_text, width - outer_margin * 2.0)
        title_height = self._compact_note_height_for_font(note_font)
        canvas_top = title_height + 16.0
        inner_width = width - outer_margin * 2.0
        note_width = inner_width
    ...
    def _active_frame_rect(self) -> QRectF:
        if self._uses_compact_card() and "frame" in self._card_layout:
            return self._card_layout["frame"]
        if self.node.type == "Initial":
            return self._card_layout.get("frame", self._initial_frame_rect())
        return self._rect

    def _offset_header_layout(self, dx: float, dy: float) -> None:
        self._title_rect.translate(dx, dy)
        self._summary_layout_rows = [
            (QRectF(rect).translated(dx, dy), text, color)
            for rect, text, color in self._summary_layout_rows
        ]

    def _paint_blueprint_edge_glow(self, painter: QPainter, rect: QRectF, color: QColor, *, strong: bool) -> None:
        glow = QColor(color)
        glow.setAlpha(92 if strong else 54)
        core = QColor(color)
    ...
    def _paint_pin_glow(self, painter: QPainter, color: QColor, *, strong: bool) -> None:
        if not self._supports_connections():
            return
        pin_glow = QColor(color)
        pin_glow.setAlpha(92 if strong else 58)
        radius = self._pin_radius + (7.0 if strong else 4.5)
        painter.setPen(Qt.PenStyle.NoPen)
        painter.setBrush(pin_glow)
        if self._supports_input_connection():
            painter.drawEllipse(self._pin_rect("input", radius))
        if self._supports_output_connection():
            painter.drawEllipse(self._pin_rect("output", radius))
    ...
    def _ui_theme_palette(self) -> ThemePalette:
        view = self._canvas_view()
        return view.theme_palette if view is not None else palette_for_theme(ThemeMode.DARK)

    @staticmethod
    def _contrast_text_color(background: QColor, preferred: QColor) -> QColor:
        ...
        if preferred.isValid() and contrast(background, preferred) >= 4.5:
            return preferred
        dark = QColor("#111827")
        light = QColor("#f8fafc")
    ...
    def _node_shell_colors(self) -> tuple[QColor, QColor, QColor, QColor, QColor]:
        ...
        if self._ui_theme_palette().mode is ThemeMode.LIGHT:
            body_color = self._mix_colors(QColor("#ffffff"), QColor(body_seed), 0.38)
            header_color = self._mix_colors(body_color, QColor(border_seed), 0.34)
            border_color = self._mix_colors(QColor("#334155"), QColor(border_seed), 0.76)
            accent = QColor(border_color)
            body_color.setAlpha(250)
            header_color.setAlpha(252)
            border_color.setAlpha(238)
            accent.setAlpha(242)
            return body_color, header_color, border_color, accent, self._contrast_text_color(body_color, text_color)
    ...
    def _comment_colors(self) -> tuple[QColor, QColor, QColor, QColor, QColor]:
        ...
        light_mode = self._ui_theme_palette().mode is ThemeMode.LIGHT
        paper_base = QColor("#fffdf7" if light_mode else "#17130d")
        accent_base = self._mix_colors(QColor(border_seed), QColor("#ffe3a8"), 0.18)
        body_color = self._mix_colors(paper_base, QColor(body_seed), 0.20 if light_mode else 0.45)
        ...
        if light_mode:
            text_color = self._contrast_text_color(body_color, text_color)
    ...
    def _compact_card_palette(self) -> dict[str, QColor]:
        ...
        light_mode = self._ui_theme_palette().mode is ThemeMode.LIGHT
        note_fill = self._mix_colors(QColor("#edf7f2" if light_mode else "#1e2c25"), accent, 0.42)
        note_fill.setAlpha(240)
        note_text = (
            self._contrast_text_color(note_fill, QColor(text_color))
            if light_mode
            else self._mix_colors(QColor("#eefbf4"), QColor(text_color), 0.14)
        )
        ...
        action_text = self._contrast_text_color(action_fill, QColor(text_color))
        ...
        parameter_text = self._contrast_text_color(parameter_fill, QColor(text_color))
        frame_fill = self._mix_colors(body_color, QColor("#ffffff" if light_mode else "#07080a"), 0.08)
        ...
        return {
            ...
            "draw_text": self._contrast_text_color(draw_fill, QColor(text_color)) if light_mode else QColor("#ffffff"),
            "action_fill": action_fill,
            "action_border": action_border,
            "action_text": action_text,
            "target_fill": target_fill,
            "target_border": target_border,
            "target_text": self._contrast_text_color(target_fill, QColor("#1f2530")),
    ...
    def _compact_text_layout(
        ...
        *,
        ...
        min_point_size = self._screen_scaled_point_size(
            min_point_size,
            minimum_screen=min_point_size,
            maximum_screen=min_point_size,
        )
        if draw_rect.height() > 1.0:
            min_point_size = min(min_point_size, max(1.0, draw_rect.height() * 0.62))
        point_size = max(min_point_size, point_size)
        min_point_size = max(1.0, min_point_size)
        ...
        if not shrink_to_fit:
            fitted_font.setPointSizeF(point_size)
            metrics = QFontMetricsF(fitted_font)
            if metrics.height() > draw_rect.height() + 0.5 and point_size > min_point_size:
                point_size = max(
                    min_point_size,
                    point_size * draw_rect.height() / max(1.0, metrics.height()),
                )
                fitted_font.setPointSizeF(point_size)
    ...
    def _paint_tool_badge_shell(self, painter: QPainter, rect: QRectF, *, active: bool = False) -> None:
        lock_fill = QColor("#20242c" if active else "#14181f")
        painter.setBrush(lock_fill)
        painter.setPen(QPen(QColor("#d4d9e6" if active else "#758098"), 1.0))
        painter.drawRoundedRect(rect, 4, 4)

    def _paint_lock_badge(self, painter: QPainter) -> None:
        rect = self.lock_rect()
        self._paint_tool_badge_shell(painter, rect, active=self.node.locked)
        shackle = QPainterPath()
        shackle.moveTo(rect.left() + 5.5, rect.top() + 8.0)
        shackle.arcTo(rect.left() + 4.0, rect.top() + 3.0, 10.0, 10.0, 180.0, -180.0)
        painter.drawPath(shackle)
        body_rect = QRectF(rect.left() + 4.5, rect.top() + 8.5, 9.0, 6.5)
        painter.drawRoundedRect(body_rect, 2, 2)

    def _paint_detail_toggle_badge(self, painter: QPainter) -> None:
        rect = self.detail_toggle_rect()
        if rect.isNull() or not rect.isValid():
            return
        self._paint_tool_badge_shell(painter, rect)
        icon_color = QColor("#9ba6bd")
        painter.setBrush(icon_color)
        painter.setPen(QPen(icon_color, 1.25, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap))
        for offset in (5.0, 9.0, 13.0):
            y = rect.top() + offset
            painter.drawEllipse(QRectF(rect.left() + 4.3, y - 1.0, 2.0, 2.0))
            painter.drawLine(QPointF(rect.left() + 8.0, y), QPointF(rect.right() - 4.0, y))
    ...
    def paint(self, painter: QPainter, option, widget=None) -> None:
        ...
        active_frame_rect = self._active_frame_rect()
        shadow_base_rect = active_frame_rect if self.node.type == "Initial" else self._rect
        shadow_color = QColor(0, 0, 0, 48 if self.isSelected() else 30)
        painter.setPen(Qt.PenStyle.NoPen)
        painter.setBrush(shadow_color)
        painter.drawRoundedRect(shadow_base_rect.adjusted(0, 4, 0, 4), 10, 10)
        should_glow = bool(not fast_render and (self.isSelected() or self.is_relation_highlighted() or self._hovered or self._search_highlight))
        glow_color = QColor("#8fd3ff") if self._hovered or self._search_highlight else (QColor("#fff4a8") if self.is_relation_highlighted() and not self.isSelected() else QColor("#f0b429"))
        if should_glow:
            self._paint_blueprint_edge_glow(painter, active_frame_rect, glow_color, strong=self.isSelected() or self._search_highlight)
            self._paint_pin_glow(painter, glow_color, strong=self.isSelected() or self._search_highlight)
        ...
        if self._uses_compact_card():
            ...
            self._paint_compact_text(
                painter,
                identity_rect,
                self._compact_identity_text(),
                self._compact_title_font(),
                self._contrast_text_color(palette["frame_inner_fill"], text_color),
        ...
        if self.node.type == "Comment":
            ...
            accent_bar.setAlpha(220)
            painter.setPen(Qt.PenStyle.NoPen)
            painter.setBrush(accent_bar)
            painter.drawRoundedRect(QRectF(12.0, 10.0, max(40.0, self._rect.width() - 24.0), 4.0), 2, 2)
            if self._comment_editor_proxy is None:
                content = str(self.node.fields.get("content", "") or "")
                draw_color = QColor(text_color)
                if not content.strip():
                    content = "双击输入备注"
                    draw_color.setAlpha(min(draw_color.alpha(), 118))
                painter.setPen(draw_color)
                painter.setFont(self._comment_content_font())
                painter.drawText(
                    self._title_rect,
        ...
        painter.fillRect(QRectF(frame_rect.left(), frame_rect.top() + self._header_height, frame_rect.width(), 12), header_color)
        accent_bar = QColor(accent)
        accent_bar.setAlpha(min(255, accent_bar.alpha() + 12))
        painter.setBrush(accent_bar)
        painter.setPen(Qt.PenStyle.NoPen)
        painter.drawRoundedRect(QRectF(frame_rect.left() + 12, frame_rect.top() + 8, max(52.0, frame_rect.width() - 24), 4), 2, 2)
        if self._is_function_node():
            self._paint_detail_toggle_badge(painter)
        self._paint_lock_badge(painter)
        content_rect = QRectF(
            self.proxy.pos().x() - 2,
    ...
    def mousePressEvent(self, event: QMouseEvent) -> None:
        ...
        if event.button() == Qt.MouseButton.LeftButton and self.detail_toggle_rect().contains(local_pos):
            view = self._canvas_view()
            if view:
                view.toggle_node_display_mode(self.node.uuid)
            event.accept()
            return
        if self.lock_rect().contains(local_pos):
            self.controller.set_node_locked(self.node.uuid, not self.node.locked)
            event.accept()
            return
        if self.schema.nodes[self.node.type].resizable and self.resize_handle_rect().contains(local_pos):
            if self.node.locked:
                event.accept()
                return
            self._resizing = True
            ...
            event.accept()
            return
        if self._proxy_contains(local_pos):
            super().mousePressEvent(event)
            return
    ...
    def mouseReleaseEvent(self, event: QMouseEvent) -> None:
        ...
        if self._group_drag_targets:
            ...
            if view:
                if not view.positions_match_document(positions) or membership_changes:
                    self.controller.move_nodes_with_group_memberships(
                        positions,
                        membership_changes,
                        label="Move frame with nodes",
                    )
                else:
    ...
    def hoverMoveEvent(self, event: QGraphicsSceneHoverEvent) -> None:
        if self.detail_toggle_rect().contains(event.pos()) or self.lock_rect().contains(event.pos()):
    ...
    def _commit_field(self, key: str, value: Any) -> None:
        self.controller.update_field(self.node.uuid, key, value, self._editor_source_mode())

    def _commit_fields(self, values: dict[str, Any]) -> None:
        self.controller.update_fields(self.node.uuid, values, self._editor_source_mode(), label="应用外观方案")

    def _editor_source_mode(self) -> str:
        return "advanced" if self._display_mode == "detail" else "simple"
    ...
    def _root_title_reserved_width(self) -> float:
        """Reserve stable space for the largest supported root-title font."""
        ...
        font.setPointSizeF(
            min(
                ...
                self.TITLE_BASE_POINT_SIZE / self.TITLE_SCALE_FLOOR,
            )
        )
        return float(QFontMetricsF(font).horizontalAdvance(self._full_title_text())) + 64.0
    ...
    def _comment_content_font(self) -> QFont:
        font = QFont(self.form.font())
        font.setBold(True)
        try:
            configured = float(self.node.fields.get("note_font_size", 15))
        except (TypeError, ValueError):
            configured = 15.0
        font.setPointSizeF(
            self._screen_scaled_point_size(
    ...
    def _compact_note_font_for_text(self, text: str, max_width: float) -> QFont:
        ...
        minimum = self._screen_scaled_point_size(
            self.CARD_NOTE_MIN_POINT_SIZE,
            minimum_screen=self.CARD_NOTE_MIN_POINT_SIZE,
            maximum_screen=self.CARD_NOTE_MIN_POINT_SIZE,
        )
        font.setPointSizeF(max(minimum, min(current_size, scaled_size)))
        return font
    ...
    def _scaled_card_editor_font(self, font: QFont) -> QFont:
        editor_font = QFont(font)
        point_size = editor_font.pointSizeF() if editor_font.pointSizeF() > 0 else float(editor_font.pointSize())
        if point_size > 0:
            editor_font.setPointSizeF(point_size * self.CARD_EDITOR_FONT_SCALE)
        return editor_font

    def _fit_card_editor_font_to_rect(self, font: QFont, edit_rect: QRectF) -> QFont:
        editor_font = QFont(font)
        point_size = editor_font.pointSizeF() if editor_font.pointSizeF() > 0 else float(editor_font.pointSize())
        if point_size <= 0:
            return editor_font
        max_by_height = max(16.0, edit_rect.height() * 0.58)
        editor_font.setPointSizeF(min(point_size, max_by_height))
        return editor_font

    def has_card_field_editor(self) -> bool:
        return self._card_editor_proxy is not None

    def has_comment_editor(self) -> bool:
        return self._comment_editor_proxy is not None

    def comment_title_contains(self, local_pos: QPointF) -> bool:
        return self.node.type == "Comment" and self._title_rect.contains(local_pos)
    ...
    def begin_comment_edit(self) -> bool:
        ...
        self._comment_editor_proxy = proxy
        self._sync_comment_editor_geometry()
        editor.submitted.connect(lambda value: self._finish_comment_edit(True, value))
        editor.cancelled.connect(lambda: self._finish_comment_edit(False, ""))
        editor.show()
        editor.setFocus(Qt.FocusReason.MouseFocusReason)
        editor.selectAll()
        self.update()
    ...
    def _finish_comment_edit(self, commit: bool, value: str) -> None:
        if self._finishing_comment_edit or self._comment_editor_proxy is None:
            return
        self._finishing_comment_edit = True
        proxy = self._comment_editor_proxy
        self._comment_editor_proxy = None
        editor = proxy.widget()
        if editor is not None:
        ...
        self._finishing_comment_edit = False
        self.update()
        if commit and value != str(self.node.fields.get("content", "") or ""):
    ...
    def _sync_card_editor_geometry(self, field_key: str, editor) -> None:
        ...
        editor.setFont(self._fit_card_editor_font_to_rect(editor.font(), edit_rect))
        self._apply_card_editor_style(editor)
        font_height = int(math.ceil(QFontMetricsF(editor.font()).height() + 10.0))
        height = max(40, int(math.ceil(edit_rect.height())), font_height)
        y = edit_rect.top() - max(0.0, (height - edit_rect.height()) / 2.0)
        ...
        self._card_editor_proxy.setPos(QPointF(edit_rect.left(), y))
        self._card_editor_proxy.setGeometry(QRectF(edit_rect.left(), y, width, height))
    ...
    def open_appearance_dialog(self, anchor_global_pos=None) -> bool:
        if self.node.locked or self.node.type in APPEARANCE_DISABLED_NODE_TYPES:
            return False
        self.setSelected(True)
        self.controller.set_selected_node(self.node.uuid)
        if self.node.type == "Comment":
            values = {
                "theme_body_color": self.node.fields.get("note_box_color") or self.node.fields.get("theme_body_color"),
                "theme_border_color": self.node.fields.get("theme_border_color"),
                "theme_text_color": self.node.fields.get("note_text_color") or self.node.fields.get("theme_text_color"),
            }
            dialog = NodeAppearanceDialog(values, self._canvas_view() or self.form)
            _move_dialog_near_anchor(dialog, anchor_global_pos)
            if dialog.exec() != QDialog.DialogCode.Accepted:
                return False
            colors = dialog.values()
            updates = {
                "note_box_color": colors["theme_body_color"],
                "theme_body_color": colors["theme_body_color"],
                "theme_border_color": colors["theme_border_color"],
                "note_text_color": colors["theme_text_color"],
                "theme_text_color": colors["theme_text_color"],
            }
    ...
    def _card_editor_font(self, field_key: str) -> QFont:
        ...
        return self._scaled_card_editor_font(self._compact_parameter_font())
    ...
    def _compact_identity_text(self) -> str:
        if self._is_function_node() or self.node.type == "ParameterTrigger":
            ...
            draw_name = str(self.node.fields.get("draw_able_name") or "").strip()
            return draw_name or f"{definition.title}{slot}"
        return self._full_title_text()
...
class GroupItem(QGraphicsObject):
    ...
    def __init__(self, controller, view: "NodeCanvasView", group) -> None:
        ...
        self._title_editor_proxy: QGraphicsProxyWidget | None = None
        self._finishing_title_edit = False
    ...
    @classmethod
    def frame_rect_for_members(cls, member_rects: list[QRectF]) -> QRectF:
        title_height = max(32.0, float(QFontMetrics(cls._title_font()).height()) + 12.0)
        top_padding = cls.TITLE_TOP_MARGIN + title_height + cls.TITLE_BOTTOM_GAP
        if not member_rects:
            return QRectF(0.0, 0.0, 180.0, 110.0)
        bounds = QRectF(member_rects[0])
        for rect in member_rects[1:]:
            bounds = bounds.united(rect)
        return QRectF(
            bounds.left() - cls.OUTER_PADDING,
            bounds.top() - top_padding,
            max(180.0, bounds.width() + cls.OUTER_PADDING * 2.0),
            max(110.0, bounds.height() + top_padding + cls.OUTER_PADDING),
        )
    ...
    def update_group(self, group, member_rects: list[QRectF]) -> None:
        ...
        if group.ui_position and group.ui_size:
            frame = QRectF(
                float(group.ui_position.get("x", 0.0)),
                float(group.ui_position.get("y", 0.0)),
        ...
        self._rect = QRectF(0.0, 0.0, frame.width(), frame.height())
        self._title_rect = QRectF(
            self.TITLE_SIDE_MARGIN,
            self.TITLE_TOP_MARGIN,
            max(120.0, frame.width() - self.TITLE_SIDE_MARGIN * 2.0),
            title_height,
        )
        self._sync_title_editor_geometry()
        self.update()
    ...
    def _sync_title_editor_geometry(self) -> None:
        if self._title_editor_proxy is not None:
            self._title_editor_proxy.setGeometry(self._title_rect.adjusted(2.0, 1.0, -2.0, -1.0))

    def begin_title_edit(self) -> bool:
        if self._title_editor_proxy is not None:
            editor = self._title_editor_proxy.widget()
            if editor is not None:
                editor.setFocus(Qt.FocusReason.MouseFocusReason)
                editor.selectAll()
            return True
        editor = InlineTitleLineEdit()
        editor.setText(self.group_title())
        editor.setFont(self._title_font())
        editor.setObjectName("inlineGroupTitleEditor")
        palette = self.view.theme_palette
        editor.setStyleSheet(
            f"QLineEdit {{ background: {palette.editor_background}; color: {palette.editor_text}; "
            f"border: 2px solid {palette.connection_selected}; border-radius: 6px; padding: 0 8px; }}"
        )
        proxy = QGraphicsProxyWidget(self)
        proxy.setZValue(120.0)
        proxy.setWidget(editor)
        self._title_editor_proxy = proxy
        self._sync_title_editor_geometry()
        editor.submitted.connect(lambda value: self._finish_title_edit(True, value))
        editor.cancelled.connect(lambda: self._finish_title_edit(False, ""))
        editor.show()
        editor.setFocus(Qt.FocusReason.MouseFocusReason)
        editor.selectAll()
        self.update()
    ...
    def _finish_title_edit(self, commit: bool, value: str) -> None:
        if self._finishing_title_edit or self._title_editor_proxy is None:
            return
        self._finishing_title_edit = True
        proxy = self._title_editor_proxy
        self._title_editor_proxy = None
        editor = proxy.widget()
        if editor is not None:
            editor.blockSignals(True)
            proxy.setWidget(None)
        ...
        self._finishing_title_edit = False
        self.update()
        if commit and value != self.group_title():
            self.controller.rename_group(self.group.uuid, value)

    def commit_pending_title_edit(self) -> None:
        if self._title_editor_proxy is None:
            return
        editor = self._title_editor_proxy.widget()
        if isinstance(editor, QLineEdit):
            self._finish_title_edit(True, editor.text())
    ...
    def mouseReleaseEvent(self, event: QMouseEvent) -> None:
        if self._dragging:
            ...
            group_position = self.view.snap_position(self.pos())
            self.setPos(QPointF(group_position[0], group_position[1]))
            delta = self.pos() - self._drag_start_pos
            positions = self.view.snap_positions(
                {node_uuid: origin + delta for node_uuid, origin in self._member_origins.items()}
            )
            self.view.apply_item_positions(positions)
            self._member_origins.clear()
            self._table_origins.clear()
            bounds = (group_position[0], group_position[1], self._rect.width(), self._rect.height())
            group = self.controller.get_group(self.group.uuid)
            old_group_position = (
                float((group.ui_position or {}).get("x", self._drag_start_pos.x())) if group else self._drag_start_pos.x(),
                float((group.ui_position or {}).get("y", self._drag_start_pos.y())) if group else self._drag_start_pos.y(),
            )
            if old_group_position != group_position or (positions and not self.view.positions_match_document(positions)):
                self.controller.move_group_with_nodes(self.group.uuid, positions, bounds)
            event.accept()
            return
        super().mouseReleaseEvent(event)
...
class ParameterTableItem(QGraphicsObject):
    ...
    def _build_columns(self) -> list[tuple[str, str, float]]:
        ...
        concise_fields = set(self.view.concise_fields) if self.view.concise_enabled else None
        for field in self.schema.nodes["ParameterTrigger"].fields:
            if "simple" not in field.show_in_modes or field.key in skip_keys:
                continue
            if concise_fields is not None and field.key not in ({"draw_able_name"} | concise_fields):
                continue
            width = 112.0
    ...
    def _palette(self) -> tuple[QColor, QColor, QColor]:
        ...
        body = QColor(colors["_table_body_color"])
        border = QColor(colors["_table_border_color"])
        text = QColor(colors["_table_text_color"])
        if self.view.theme_mode is ThemeMode.LIGHT:
            body = self._mix_colors(QColor("#ffffff"), body, 0.44)
            border = self._mix_colors(QColor("#334155"), border, 0.76)
            text = NodeItem._contrast_text_color(body, text)
        return body, border, text
    ...
    def commit_pending_edit(self) -> bool:
        proxy = self._editor_proxy
        editor = proxy.widget() if proxy is not None else None
        if editor is None or self._editor_target is None:
            return False
        emit_commit = getattr(editor, "_emit_commit", None)
        if not callable(emit_commit):
            return False
        emit_commit()
        return True
    ...
    def _commit_cell_edit(self, value: Any, editor) -> None:
        ...
        # Parameter-table cells are literal configuration values. Treat them as
        # advanced edits so auto-linking never rewrites what the user entered.
        self.controller.update_field(row_uuid, field_key, value, "advanced")
    ...
    def paint(self, painter: QPainter, option, widget=None) -> None:
        ...
        for _field_key, label, width in self._columns:
            ...
            painter.setPen(NodeItem._contrast_text_color(header_fill, QColor(text_color)))
    ...
    def mouseReleaseEvent(self, event: QMouseEvent) -> None:
        if self._dragging:
            ...
            if (positions and not self.view.positions_match_document(positions)) or membership_changes:
                self.controller.move_nodes_with_group_memberships(
                    positions,
                    membership_changes,
                    label="Move parameter table",
                )
            else:
...
class NodeCanvasView(QGraphicsView):
    ...
    def __init__(self, schema: EditorSchema, controller, parent=None) -> None:
        ...
        self.image_items: dict[str, CanvasImageItem] = {}
        self.connection_items: dict[tuple[str, str], ConnectionItem] = {}
        self._connection_pairs_by_node: dict[str, set[tuple[str, str]]] = defaultdict(set)
        ...
        self.concise_enabled = False
        self.concise_fields: set[str] = {"tips", "action_trigger", "action_trigger_active"}
        self.concise_elements: dict[str, bool] = {
            "groups": False,
            "tables": False,
            "images": False,
            "strokes": False,
        ...
        self._detailed_connection_effects = True
        ...
        self._temp_path = TemporaryConnectionItem(self)
        ...
        controller.canvasImagesChanged.connect(self._rebuild_canvas_images)
        canvas_strokes_changed = getattr(controller, "canvasStrokesChanged", None)
        if canvas_strokes_changed is not None:
            canvas_strokes_changed.connect(self._rebuild_canvas_strokes)
        self.rebuild_scene()

    def set_ui_theme(self, mode: ThemeMode | str) -> None:
        resolved = normalize_theme_mode(mode)
        if resolved is self.theme_mode:
            return
        self.theme_mode = resolved
        self.theme_palette = palette_for_theme(resolved)
        self.scene_ref.set_ui_theme(resolved)
    ...
    def rebuild_scene(self) -> None:
        with performance_recorder.measure(
            ...
            for item in list(self.node_items.values()):
                self.scene_ref.removeItem(item)
            for item in list(self.image_items.values()):
                self.scene_ref.removeItem(item)
            for item in list(self.stroke_items.values()):
                self.scene_ref.removeItem(item)
            self.connection_items.clear()
            self._connection_pairs_by_node.clear()
            ...
            self._rebuild_canvas_images()
            self._rebuild_canvas_strokes()
    ...
    def set_pen_style(self, color: str, width: float) -> None:
        ...
        self.pen_width = 2.0 if float(width) <= 2.0 else (4.0 if float(width) <= 4.0 else 8.0)
        if self._drawing_stroke:
            self._temporary_stroke.setPen(
                QPen(QColor(self.pen_color), self.pen_width, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap, Qt.PenJoinStyle.RoundJoin)
            )

    def set_concise_display(
        self,
        enabled: bool,
        fields: set[str] | list[str] | tuple[str, ...] | None = None,
        elements: dict[str, bool] | None = None,
    ) -> None:
        self.concise_enabled = bool(enabled)
        if fields is not None:
            self.concise_fields = {
                key
                for key in fields
                if key in {"tips", "draw_able_name", "parameter", "action_trigger", "action_trigger_active"}
            }
        if elements is not None:
            for key in self.concise_elements:
                if key in elements:
                    self.concise_elements[key] = bool(elements[key])
        if self.concise_enabled:
            self.expanded_node_uuids.clear()
        self.rebuild_scene()

    def _concise_element_visible(self, key: str) -> bool:
        return not self.concise_enabled or bool(self.concise_elements.get(key, False))
    ...
    def should_use_fast_rendering(self) -> bool:
        item_count = len(self.node_items) + len(self.table_items) + len(self.image_items)
        if item_count <= 50:
            return False
    ...
    def should_render_parameter_rows_as_nodes(self) -> bool:
        return self.concise_enabled and not bool(self.concise_elements.get("tables", False))
    ...
    def connection_pairs_for_node(self, node_uuid: str) -> frozenset[tuple[str, str]]:
        return frozenset(self._connection_pairs_by_node.get(node_uuid, ()))

    def connection_flow_phase(self) -> float:
        return self._connection_flow_phase

    def should_use_detailed_connection_effects(self) -> bool:
        return self._detailed_connection_effects

    def _handle_selection_changed(self, _uuid: str | None) -> None:
        self._refresh_node_z_values()
        self._refresh_relation_highlights()

    def _active_flow_connection_pairs(self) -> set[tuple[str, str]]:
        active = set(self._related_connection_pairs)
        for pair, item in self.connection_items.items():
            if item.isSelected():
                active.add(pair)
        return active

    def _refresh_blueprint_animation_timer(self) -> None:
        active_pairs = self._active_flow_connection_pairs()
        self._detailed_connection_effects = len(active_pairs) <= self.DETAILED_CONNECTION_EFFECT_LIMIT
        should_run = bool(self._connecting_from or active_pairs)
        if should_run and not self._connection_flow_timer.isActive():
            self._connection_flow_timer.start()
        elif not should_run and self._connection_flow_timer.isActive():
    ...
    def _advance_connection_flow(self) -> None:
        ...
        for pair in self._active_flow_connection_pairs():
            item = self.connection_items.get(pair)
            if item is not None:
                item.update()
        if self._connecting_from and self._temp_path.isVisible():
            self._temp_path.update()
        self._refresh_blueprint_animation_timer()
    ...
    def reset_view_layout(self) -> None:
        ...
        for item in self.group_items.values():
            ...
            item_rect = item.mapRectToScene(item.boundingRect())
            target_rect = item_rect if target_rect is None else target_rect.united(item_rect)
        for item in self.stroke_items.values():
            if not item.isVisible():
                continue
            item_rect = item.mapRectToScene(item.boundingRect())
            target_rect = item_rect if target_rect is None else target_rect.united(item_rect)
    ...
    def _append_stroke_point(self, scene_point: QPointF, *, force: bool = False) -> None:
        if self._drawing_points and not force:
            ...
            if math.hypot(current_view.x() - previous_view.x(), current_view.y() - previous_view.y()) < 2.0:
                return
        self._drawing_points.append(QPointF(scene_point))
        self._temporary_stroke.setPath(CanvasStrokeItem.path_for_points(self._drawing_points))
    ...
    def cancel_stroke_preview(self) -> None:
        if self._drawing_stroke:
            self._set_interaction_busy("stroke", False)
        self._drawing_stroke = False
        self._drawing_points = []
    ...
    def _finish_stroke_preview(self, scene_point: QPointF) -> None:
        ...
        add_stroke = getattr(self.controller, "add_canvas_stroke", None)
    ...
    def mouseDoubleClickEvent(self, event: QMouseEvent) -> None:
        if event.button() == Qt.MouseButton.LeftButton:
            group_item = self._group_item_at_view_point(event.position())
            if group_item:
                scene_pos = self.mapToScene(event.position().toPoint())
                if group_item.title_contains(scene_pos):
                    group_item.begin_title_edit()
                    event.accept()
                    return
            node_item = self._node_item_at_view_point(event.position())
            if node_item:
    ...
    def contextMenuEvent(self, event: QContextMenuEvent) -> None:
        ...
        if node_item:
            ...
            if node_item.has_card_field_editor():
                node_item._discard_card_field_editor()
            if node_item.node.type == "Comment":
                node_item.open_appearance_dialog(event.globalPos())
                event.accept()
                return
            if node_item.isSelected() and len(self.selected_node_uuids()) > 1:
                self._edit_selected_node_appearance(event.globalPos())
                event.accept()
                return
            if node_item.node.type not in APPEARANCE_DISABLED_NODE_TYPES:
                node_item.open_appearance_dialog(event.globalPos())
            event.accept()
            return
    ...
    def _create_item(self, node) -> None:
        if node.type == "DrawFrame":
            return
        if node.type == "ParameterTrigger" and not self.should_render_parameter_rows_as_nodes():
            return
        ...
        # The item can only resolve concise display state after it belongs to
        # this view's scene; refresh once more so its compact layout is built.
    ...
    def _add_or_update_node_item(self, node_uuid: str) -> None:
        ...
        if not node:
            return
        if node.type == "DrawFrame" or (
            node.type == "ParameterTrigger" and not self.should_render_parameter_rows_as_nodes()
        ):
            self._rebuild_parameter_tables()
            self._rebuild_groups()
            self._update_connections_for_node(node_uuid)
    ...
    def _update_node_item(self, node_uuid: str) -> None:
        ...
        if node and (
            node.type == "DrawFrame"
            or (node.type == "ParameterTrigger" and not self.should_render_parameter_rows_as_nodes())
        ):
            self._rebuild_parameter_tables()
            self._rebuild_groups()
            self._update_connections_for_node(node_uuid)
    ...
    def _remove_node_item(self, node_uuid: str) -> None:
        ...
        for pair in list(self.connection_items.keys()):
            if node_uuid in pair:
                ...
                for endpoint_uuid in pair:
                    endpoint_pairs = self._connection_pairs_by_node.get(endpoint_uuid)
                    if endpoint_pairs is not None:
                        endpoint_pairs.discard(pair)
                        if not endpoint_pairs:
                            self._connection_pairs_by_node.pop(endpoint_uuid, None)
        self._rebuild_parameter_tables()
        self._rebuild_groups()
    ...
    def _rebuild_connections(self) -> None:
        with performance_recorder.measure(
            ...
            for connection in self.controller.document.connections:
                ...
                pair = (connection.from_uuid, connection.to_uuid)
                self.connection_items[pair] = item
                self._connection_pairs_by_node[connection.from_uuid].add(pair)
                self._connection_pairs_by_node[connection.to_uuid].add(pair)
                self.scene_ref.addItem(item)
            self._refresh_relation_highlights()

    def _rebuild_canvas_images(self) -> None:
        selected = set(self.selected_canvas_image_uuids())
        for item in list(self.image_items.values()):
            self.scene_ref.removeItem(item)
        self.image_items.clear()
        for record in self.controller.document.canvas_images:
            item = CanvasImageItem(self.controller, self, record)
            self.scene_ref.addItem(item)
            self.image_items[record.uuid] = item
            item.setVisible(self._concise_element_visible("images"))
    ...
    def _rebuild_parameter_tables(self) -> None:
        ...
        for table in self.controller.parameter_tables():
            ...
            item.setVisible(self._concise_element_visible("tables"))
            for row in rows:
    ...
    def _rebuild_groups(self) -> None:
        ...
        for group in groups:
            ...
            if member_rects and not (group.ui_position and group.ui_size):
                migrated_frame = GroupItem.frame_rect_for_members(member_rects)
                migrated = self.controller.initialize_group_geometry(
                    group.uuid,
                    (migrated_frame.x(), migrated_frame.y(), migrated_frame.width(), migrated_frame.height()),
                )
                if migrated is not None:
                    group = migrated
            if not member_rects and not (group.ui_position and group.ui_size):
                continue
            ...
            item.setVisible(self._concise_element_visible("groups"))

    def group_bounds_for_nodes(self, node_uuids: list[str]) -> QRectF | None:
        member_rects: list[QRectF] = []
        processed_tables: set[str] = set()
        for node_uuid in node_uuids:
            table_item = self.table_row_to_item.get(node_uuid)
            ...
            if rect is not None:
                member_rects.append(rect)
        return GroupItem.frame_rect_for_members(member_rects) if member_rects else None

    def fit_group_to_contents(self, group_uuid: str) -> bool:
        frame = self.group_bounds_for_nodes(self.controller.group_node_uuids(group_uuid))
        if frame is None:
            return False
        return self.controller.set_group_geometry(
            group_uuid,
    ...
    def group_for_node_drop(self, node_uuid: str) -> str | None:
        node = self.controller.get_node(node_uuid)
        if not node or node.type in {"Initial", "Idle0"}:
            return None
        node_rect = self.node_visual_rect(node_uuid)
        if node_rect is None:
            return None
        node_center = node_rect.center()
        node_area = max(1.0, node_rect.width() * node_rect.height())
        matches: list[tuple[float, float, str]] = []
        for group_uuid, group_item in self.group_items.items():
            if not group_item.isVisible():
                continue
            group_rect = group_item.focus_rect()
            overlap = node_rect.intersected(group_rect)
            overlap_area = max(0.0, overlap.width()) * max(0.0, overlap.height())
            overlap_ratio = overlap_area / node_area
            if overlap_ratio < 0.25 and not group_rect.contains(node_center):
                continue
            group_area = max(1.0, group_rect.width() * group_rect.height())
            matches.append((-overlap_ratio, group_area, group_uuid))
        if not matches:
            return None
    ...
    def _update_connections_for_node(self, node_uuid: str) -> None:
        for pair in self._connection_pairs_by_node.get(node_uuid, ()):
            item = self.connection_items.get(pair)
            if item is not None:
                item.update_path()
                item.update()
    ...
    def _on_node_geometry_changed(self, node_uuid: str) -> None:
        if "resize" in self._interaction_flags and self.should_use_motion_preview():
    ...
    def _update_temp_connection(self, end_scene_pos: QPointF) -> None:
        ...
        if start is None:
            return
        target_uuid = self._input_target_uuid_at_scene(end_scene_pos, exclude_uuid=self._connecting_from)
        if target_uuid:
            target_anchor = self.connection_anchor_scene_pos(target_uuid, "input")
            if target_anchor is not None:
                end_scene_pos = target_anchor
        self._temp_path.set_target_active(bool(target_uuid))
        self._temp_path.update_path(start, end_scene_pos)
    ...
    def cancel_connection_preview(self) -> None:
        ...
        self._temp_path.hide()
        self._temp_path.set_target_active(False)
    ...
    def _appearance_editable_selected_nodes(self) -> list:
        nodes = [self.controller.get_node(node_uuid) for node_uuid in self.selected_node_uuids()]
        return [
            node
            for node in nodes
            if node is not None and not node.locked and node.type not in APPEARANCE_BUTTON_HIDDEN_NODE_TYPES
        ]

    def _edit_selected_node_appearance(self, global_pos=None) -> bool:
        editable_nodes = self._appearance_editable_selected_nodes()
        if not editable_nodes:
            return False
        dialog = NodeAppearanceDialog(editable_nodes[0].fields, self)
        if global_pos is not None:
            _move_dialog_near_anchor(dialog, global_pos)
        if dialog.exec() != QDialog.DialogCode.Accepted:
            return False
    ...
    def _handle_document_state_changed(self, state) -> None:
        self.scene_ref.top_right_hint = "" if state.is_meta_ready else "!!当前文件缺少配置底座元数据!!"
        self._refresh_hint_overlay()

    def _refresh_hint_overlay(self, *_args) -> None:
        self.scene_ref.top_right_hint = (
            "" if self.controller.document.state.is_meta_ready else "!!当前文件缺少配置底座元数据!!"
        )
        tips = str(self.controller.document.meta.tips or "").strip()
        self.scene_ref.bottom_right_hint = tips or "按住中键平移 / 滚轮缩放 / 右键创建 / Ctrl+V 粘贴截图 / Delete 删除"
        self.scene_ref.update()
