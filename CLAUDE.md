...
**Document model (`models.py`)** — plain dataclasses describe metadata, nodes, connections, persistent groups, reference images, separate formal/plan stroke layers, plan layout and settings. Documents with listener subgraphs use v6; ordinary documents still write v5. v1-v5 remain readable and newer unknown formats are rejected. `manual_fields` records overrides of generated fields. Commands clone records for undo. Legacy trash data is ignored; do not reintroduce it.

**Listener subgraphs (`listener_graph.py`, `listener_catalog.py`, `listener_compiler.py`, `listener_editor.py`)** — an optional `NodeRecord.listener_graph` owns typed child components, wires and positions. A standalone `Listener` exports one type-7 interaction; existing function nodes may attach one while preserving their own action fields. Only the compiler generates `listener_data`, `parameter`, `range`, `start_value`, `save_parameter`. Graph editing uses `SetListenerGraphCommand` on the main undo stack; copies remap internal IDs and plan materialization must never retype these hosts. Draft graphs save to JSON but strict CSV export rejects compiler errors before writing. Legacy Lua tables are parsed as literals, never executed. See `docs/LISTENER_BLUEPRINTS.md` for the fixed runtime reference and exact event/ordering semantics.

**Disk baseline (`file_tracking.py`)** — document disk stamps/digests are runtime save bookkeeping, not undoable graph content. Snapshot-replacement commands must preserve this bookkeeping and the opaque legacy `history` dictionary from the live document. Saving never generates, appends or replays embedded history. Only a successful atomic write advances the disk baseline. `MainWindow` watches the current file and parent directory, with a stat-only polling fallback; clean documents reload automatically, while drafts/external conflicts block overwrite saves.

**Graphical history (`history_view.py`, `svn_diff_dialog.py`)** — compare two committed SVN revisions through an isolated, read-only `GraphComparisonWidget`. Default to the two latest commits for this file; never use the working copy or current editor content as either endpoint. Do not attach preview items to the live editor controller. Added/deleted/modified objects retain their identity so the change list can locate nodes, connections, groups, images and strokes. Legacy embedded history is ignored by comparison. Initial fitting waits for the visible viewport layout; zoom and background panning must work before selecting any change.
