...
面向 Live2D 交互配置的 Windows 节点式编辑器。1.4.2 使用 Python
3.13.14、PySide6 6.11.1 和动态 Qt 共享库，支持 Windows 10/11 x64。
本版本的升级要点和兼容提示见
[`docs/RELEASE_NOTES_1.4.2.md`](docs/RELEASE_NOTES_1.4.2.md)。

## 安装与启动

普通用户安装：

- `L2DConfigEditor-Setup-1.4.2-x64.exe`：一个安装包同时安装两个独立
...
- 顶部 `Idle`、`Drag` 按钮用于设置计划节点类型。正式图中可多选功能节点并点击金色
  `固定`，保留其当前序号；再次点击会取消固定。重新转换时，其他节点会自动绕开固定
  序号。固定节点在正式图显示淡绿色边框，在计划图显示金色边框。
...
- 工具栏“版本历史…”在只读图表中对比保存版本：绿/红/金色分别表示新增、删除、
  修改，点击修改项定位并查看完整前后值，可切换正式图与计划图。历史以增量形式
  内嵌 JSON，最多保留 50 版、512 KiB，不重复存储未变化的图片。
  “工具 → SVN 历史版本…”可继续读取更早的提交并用同一画布对比。
...
可选顶层 `history` 扩展保存有上限的版本历史，图表格式仍为 v5。没有历史的
v1-v5 文件正常读取，后续保存建立基线；原有 SVN 历史不会自动复制到 JSON。
历史区超限时裁剪最旧版本；一次修改过大时可能只保留当前基线。损坏或与正文
不匹配的历史不会用于还原图表。旧编辑器仍可读取 v5 图表，但再次保存可能
移除历史扩展；未知的新历史扩展会原样保留。实现和兼容说明见
...
`L2DConfigEditor-Setup-1.4.2-x64.exe` 覆盖安装最简单，不必先启动 Host。
